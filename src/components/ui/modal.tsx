      ...
      {open && (
        ...
        >
          <motion.div
            ...
            className={cn(
              "bg-card border-border relative flex w-full max-w-lg flex-col overflow-hidden rounded-2xl border shadow-2xl cursor-default",
              "max-h-[90dvh]",
              className,
            )}
          >
...
export function ModalFooter({ children, className }: { children: React.ReactNode; className?: string }) {
  return <div className={cn("flex shrink-0 items-center justify-end gap-2 border-t border-border p-5 bg-card", className)}>{children}</div>;
}
