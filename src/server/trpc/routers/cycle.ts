 ...
 * same dates. The gendered wording is resolved where it is actually shown (the
 * calendar router and the reminder cron), so this one does not need to know who
 * is asking — and is spared a user lookup on every open of the panel.
 *
...
export const cycleRouter = router({
  /** The stored dates and the figure they support. */
  get: protectedProcedure.query(async ({ ctx }) => {
    await connectToDatabase();
    const log = await CycleLogModel.findOne({ spaceId: ctx.spaceId })
      .select("periodStarts")
