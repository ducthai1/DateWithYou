 ...
 *
 * `getPosition` reads the playhead at the moment of inviting, so the partner
 * starts where the host actually is rather than at 0.
 */
export function ListenTogetherControls({
  ...
  const invite = () => {
    ...
    void listen.start(tracks, index, getPosition());
  };
  ...
  if (listen.live) {
    return (
      <div className="pointer-events-auto flex w-full items-center gap-1.5">
        <span className={cn("bg-accent text-accent-foreground flex min-w-0 flex-1 items-center gap-1.5 rounded-full", pad)}>
          <Headphones className={cn("shrink-0", size === "sm" ? "h-3.5 w-3.5" : "h-4 w-4")} aria-hidden="true" />
          <span className={cn("truncate font-bold", text)}>Đang nghe cùng {partnerName}</span>
        </span>
        <button
          type="button"
          onClick={() => void listen.end()}
          className={cn("text-muted-foreground hover:text-destructive shrink-0 rounded-full px-2 py-1.5 font-semibold transition-colors", text)}
        >
          Dừng
        </button>
      </div>
  ...
  if (listen.waiting) {
    return (
      <div className="pointer-events-auto flex w-full items-center gap-1.5">
        <span className={cn("bg-muted text-muted-foreground flex min-w-0 flex-1 items-center gap-1.5 rounded-full", pad)}>
          <Loader2 className={cn("shrink-0 animate-spin", size === "sm" ? "h-3.5 w-3.5" : "h-4 w-4")} aria-hidden="true" />
          <span className={cn("truncate font-semibold", text)}>Đang chờ {partnerName} trả lời…</span>
        </span>
        <button
          type="button"
          onClick={() => void listen.end()}
